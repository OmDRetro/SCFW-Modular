...
struct hvca_h{
	...
	char filename[32]; //Check HVCA_SCFW_Conversion_v2 in SuperCardSD_Stuff/Conversion/
	char ext[4];
	long filesize;
};
...
bool filter_game(struct dirent *dirent) {
	...
	if (namelen > 4 && !strcasecmp(dirent->d_name + namelen - 4, ".fds"))
		return true;
...
void selectFile(char *path) {
	...
	} else if (pathlen > 4 && !strcasecmp(path + pathlen - 4, ".fds")){
		u32 romsize = 0;
		total_bytes = 0,bytes = 0;
		FILE *emu = fopen("/scfw/hvca.gba", "rb");
		if (!emu) {
			iprintf("FDS emu not found!\n");
			do {
				scanKeys();
				pressed = keysDownRepeat();
				VBlankIntrWait();
			...
			FILE *out_f5 = fopen("/scfw/hvca/mapr/mfds.bin", "rb");
			...
			head0.ext[0] = 0;
			head0.filesize = 0;
			FILE *out_f9 = fopen("/scfw/hvca/emu_end.dat", "wb");
			fwrite(&head0,1, sizeof head0, out_f9);
			fclose(out_f9);
			out_f9 = fopen("/scfw/hvca/emu_end.dat", "rb");
...
void change_settings(char *path) {
	for (int cursor = 0;;) {
		iprintf("\x1b[2J"
		        "SCFW Kernel v0.5.2-HVCA GBA-mode\n\n");
...
int main() {
	...
	iprintf("SCFW Kernel v0.5.2-HVCA GBA-mode\n\n");
